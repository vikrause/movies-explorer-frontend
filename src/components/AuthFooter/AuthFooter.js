...
export default function AuthFooter({ buttonText, text, linkText, path, isButtonDisabled }) {
    const location = useLocation();

    return (
        <section className={`auth-footer ${location.pathname === "/signup" ? "auth-footer_place_signup" : ""}`}>
            <button className="auth-footer__button hover-button" type="submit" disabled={isButtonDisabled}>
                {buttonText}
            </button>
